...
//import net_utils.NetIdentity;

/*
 *
 ce serveur traite simultanément tous les clients

 ce serveur comprend le langage suivant:

 date => il retourne la date
 circle x y radius => il dessine un cercle de rayon radius à la coordonnée x y
 rectangle => il dessine ...
 line => il dessine ...
 stop-server => il s'arrête
 stop ou bye => il termine la session en cours
...
public class ThreadedServer implements IServerControler {
	...
	private void removeAllServices() {
		synchronized (services) {
			serverGui.log("arrêt des services");
			clog("arrêt des services");
			if (!services.isEmpty()) {
	...
	private void stopServer() throws IOException {
		if (!socketEcoute.isClosed()) {
			serverGui.log("arrêt du serveur");
			serverEnd = true;
			removeAllServices();
			serverGui.log("arrêt de l'écoute");
			socketEcoute.close(); // débloque socketEcoute.accept() avec une
	...
	private boolean handleGraphicalRequest(PrintStream output, String request) {
		if (request.startsWith("circle") || 
				...
				|| request.startsWith("line")
				|| request.startsWith("servo")
				) {
			try {
				model.add(request);
				...
				// e.printStackTrace();
				serverGui.setError("[7] " + e.toString());
			}
		} else {
	...
	@Override
	public List<String> getModel() { // le modèle est alimenté par les clients
		return model;
	}
	...
	class EcouteThread extends Thread {
		...
		public void run() {
			try {
				...
				String host = "127.0.0.1";//HostAdress.getHostAddress();// socketEcoute.getInetAddress().getLocalHost().getHostAddress();
				int port = socketEcoute.getLocalPort();
				if (serverGui != null) {
					serverGui.setHost(host, port);
					serverGui
							.log("[serveur multiclient multisession] démarré sur :"
									+ host + ":" + port);
				}
				int connectId = 0;
				while (!serverEnd) {
					...
					if (!serverEnd) {
						if (socketService != null) {
							if (serverGui != null)
								serverGui.log("démarrage de la session ("
										+ (connectId - 1) + ")");
	...
	class ServiceThread extends Thread {
		...
		private void close(boolean verbose) {
			try {
				serverGui.log("arrêt du service pour le client "
						+ remoteSocketAddress);
				socketService.close();
				// socketService = null;
			} catch (IOException e) {
		...
		private void handleRequests() {
			while (checkConnection()) {
				...
				serverGui.log("attente requête " + remoteSocketAddress);
				try {
				...
				serverGui.log("le client " + remoteSocketAddress + " demande: "
						+ requeteclient);
				if (requeteclient.toLowerCase().contains("stop-server")) {
					...
					if (req[0].toLowerCase().equals("date")) {
						// requête attendue: date
						Date now = new Date();
						output.println("bonjour " + remoteSocketAddress
								+ " il est " + now.toString());
					} else {
						boolean ok = handleGraphicalRequest(output,
								requeteclient);
						if (ok && broadCast) {
							broadcast(this, requeteclient);
							broadcast(this, "end");
						}
